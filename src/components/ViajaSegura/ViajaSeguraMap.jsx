...
export default function MapComponent({ t }) { // Recibe t como prop
  ...
  useEffect(() => {
    ...
    const showPopup = (e, type) => {
      ...
      if (type === 'ruta') {
        ...
        html += `<div style="${titleStyle} color:${routeColor}">${t.map.popups.ruta} ${props.origen_destino}</div>
                 <div style="${rowStyle}"><span style="${labelStyle}">${t.map.popups.demanda}:</span> <span style="${valStyle}">${props.Demanda_Diaria}</span></div>
                 <div style="${rowStyle}"><span style="${labelStyle}">${t.map.popups.longitud}:</span> <span style="${valStyle}">${longitud} km</span></div>`;
      } 
      else if (type === 'parada') {
        html += `<div style="${titleStyle} color:${RAMP.descensos}">${t.map.popups.parada}</div>
                 <div style="margin-bottom:4px; font-weight:bold;">${props.origen_destino}</div>
                 <div style="${rowStyle}"><span style="${labelStyle}">${t.map.popups.suben}:</span> <span style="${valStyle}">${props.ascensos}</span></div>
                 <div style="${rowStyle}"><span style="${labelStyle}">${t.map.popups.bajan}:</span> <span style="${valStyle}">${props.descensos}</span></div>`;
      } 
      else if (type === 'equip') {
        let titleColor = RAMP.equipamiento.otros;
        let labelTrad = props.equipamiento;
        if (props.equipamiento === 'EDUCATIVO') { titleColor = RAMP.equipamiento.educativo; labelTrad = t.map.popups.educativo; }
        if (props.equipamiento === 'SALUD') { titleColor = RAMP.equipamiento.salud; labelTrad = t.map.popups.salud; }
        if (props.equipamiento === 'ABASTO') { titleColor = RAMP.equipamiento.abasto; labelTrad = t.map.popups.abasto; }
  ...
  }, [t]); // Se vuelve a ejecutar si t cambia para actualizar popups
  ...
  return (
    <div style={{ position: 'relative', width: '100%', height: '100%' }}>
      ...
      <div style={STYLES.legendBox}>
        ...
        <div style={subtitleStyle}>{t.map.recorridos}</div>
        ...
        <div style={{...itemStyle, marginTop:'4px', marginLeft: '0'}}>
            ...
            <span style={{ fontWeight: '500' }}>{t.map.isocronas}</span>
        </div>

        <div style={subtitleStyle}>{t.map.equip}</div>
        <div style={itemStyle}><span style={{...dot, background: RAMP.equipamiento.educativo}}></span> {t.map.popups.educativo}</div>
        <div style={itemStyle}><span style={{...dot, background: RAMP.equipamiento.salud}}></span> {t.map.popups.salud}</div>
        <div style={itemStyle}><span style={{...dot, background: RAMP.equipamiento.abasto}}></span> {t.map.popups.abasto}</div>
      </div>
    </div>
  );
}